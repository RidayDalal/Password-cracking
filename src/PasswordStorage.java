...
/**
 * This class models src.Password Storage in the form of a Binary Search Tree
 */
public class PasswordStorage {
     ...
     * @param comparisonCriteria, the src.Attribute that will be used to determine order in the tree
     */
     ...
     * @return the src.Attribute that is being used to make comparisons in the tree
     */
    ...
    /**
     * Provides in-order String representation of this BST, with each src.Password on its own line. The
    ...
    private boolean isValidBSTHelper(PasswordNode currentNode, Password lowerBound, Password upperBound) {
        ...
        // BASE CASE 2: the current src.Password is outside of the upper OR lower bound for this subtree, which is against
        ...
        //                   currentNode's src.Password; return false if this property is NOT satisfied

        if (!isValidBSTHelper(currentNode.getLeft(), lowerBound, currentNode.getPassword())) {
            return false;
        }

        // RECURSIVE CASE 2: Check that the right subtree contains only Passwords greater than currentNode's src.Password
     ...
     * @return the src.Password that matches the search criteria, if it does not exist in the tree it this will be null
     */
    ...
    private Password lookupHelper(Password key, PasswordNode currentNode) {
        ...
        if (key.compareTo(currentNode.getPassword(), COMPARISON_CRITERIA) < 0) {
            foundPassword = lookupHelper(key, currentNode.getLeft());
        } else if (key.compareTo(currentNode.getPassword(), COMPARISON_CRITERIA) > 0) {
            foundPassword = lookupHelper(key, currentNode.getRight());
        }
    ...
    public Password getBestPassword() {
        PasswordNode currNode = this.root;
    ...
    /**
     * Adds the src.Password to this BST.
    ...
    public void addPassword(Password toAdd) {
        PasswordNode someNode = new PasswordNode(toAdd);
        if (this.root == null) {
            root = someNode;

        } else if (!addPasswordHelper(toAdd, this.root)) {
    ...
    private boolean addPasswordHelper(Password toAdd, PasswordNode currentNode) {
        ...
        int comparisonVariable = toAdd.compareTo(currentNode.getPassword(), COMPARISON_CRITERIA);
    ...
    public void removePassword(Password toRemove) {
        if (lookup(toRemove) == null) throw new NoSuchElementException("src.Password not found");
     ...
     * @return the src.PasswordNode representing the NEW root of this subtree now that toRemove
    ...
    private PasswordNode removePasswordHelper(Password toRemove, PasswordNode currentNode) {
        ...
        //otherwise we found the node to remove!
            ...
            if (currentNode.isLeafNode()) return null;
            ...
            //1)Find the predecessor password [HINT: Write a private helper method!]
                ...
                Password predPass = findPredecessorNode(currentNode).getPassword();
                //2)Make new node for the predecessor password. It should have same left and right
                // subtree as the current node.
                PasswordNode replaceCurrent = new PasswordNode(predPass, currentNode.getLeft(),
                        currentNode.getRight());
                //3)Replace currentNode with the new predecessor node
                PasswordNode nodeToRemove = currentNode;
                currentNode = replaceCurrent;
                ...
                currentNode.setLeft(removePasswordHelper(predPass, currentNode.getLeft()));
            }
        }
        return currentNode;
    }
    ...
    /**
     * Search for the node with the password closest to, but less than the node that is passed
     * on to this method as a parameter
     *
     * @param someNode (Node for which we have to find the immediate lower password node)
     * @return the node containing whose password is just below the current password node
     */
    private PasswordNode findPredecessorNode(PasswordNode someNode) {
        PasswordNode nodeNext = someNode.getLeft();
        if (nodeNext.getRight() != null) {
            PasswordNode predecessor = nodeNext.getRight();
